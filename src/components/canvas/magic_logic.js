...
const preload = () => {
	...
	// If resources are already loaded, create environment immediately
	if (fontResource && particleResource) {
		environmentInstance = new Environment(fontResource, particleResource);
		return;
	}
	...
	if (!manager) {
		...
		loader.load('https://res.cloudinary.com/dydre7amr/raw/upload/v1612950355/font_zsd4dr.json', function (font) { 
			fontResource = font;
			if (fontResource && particleResource && document.querySelector('#magic')) {
				environmentInstance = new Environment(fontResource, particleResource);
			}
		});
		
		const particleLoader = new THREE.TextureLoader(manager);
		particleLoader.load('https://res.cloudinary.com/dfvtkoboz/image/upload/v1605013866/particle_a64uzf.png', function(particle) {
...
// Export function to allow re-initialization
if (typeof window !== 'undefined') {
	window.initializeMagicEnvironment = preload;
}
